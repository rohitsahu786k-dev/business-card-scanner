...
import { COUNTRY_CENTROIDS, WORLD_BOX } from '@/lib/world-map';
...
// Where a contact sits on the INDIA map. Returns { lat, lng, level } or null.
// Anything outside India is left for the world map — placing it here is what put
// a contact in Munich on India's centroid.
export function resolveCoordinates(city, state, country) {
  const co = norm(country);
  if (co && !INDIA_NAMES.has(co)) return null;
  ...
  // Known to be in India, but at a place we cannot pin. Note this needs the
  // country to actually SAY India: falling back on an unrecognised city alone
  // dropped a contact in Munich onto India's centroid.
  if (INDIA_NAMES.has(co)) return { lat: INDIA_CENTROID[0], lng: INDIA_CENTROID[1], level: 'country' };
  return null;
}

const INDIA_NAMES = new Set(['india', 'bharat', 'in', 'ind', 'republic of india']);

// Free-text country names as they come off a business card, mapped to the names
// Natural Earth uses in COUNTRY_CENTROIDS.
const COUNTRY_ALIASES = {
  ...
  ksa: 'Saudi Arabia', 'south korea': 'South Korea', korea: 'South Korea',
  'republic of korea': 'South Korea', 'north korea': 'North Korea',
  'czech republic': 'Czechia', czechia: 'Czechia', holland: 'Netherlands',
  netherlands: 'Netherlands', 'the netherlands': 'Netherlands',
  russia: 'Russia', 'russian federation': 'Russia',
  bharat: 'India', india: 'India', 'republic of india': 'India',
  'p.r. china': 'China', prc: 'China', 'peoples republic of china': 'China',
  hk: 'Hong Kong', sg: 'Singapore',
  vietnam: 'Vietnam', 'viet nam': 'Vietnam', turkey: 'Turkey', turkiye: 'Turkey',
  'ivory coast': "Côte d'Ivoire", myanmar: 'Myanmar', burma: 'Myanmar',
};

// City states and small islands are below the source map's minimum drawn size, so
// they carry no centroid of their own. A bubble still needs to land on them.
...
const centroidFor = (name) => COUNTRY_CENTROIDS[name] || EXTRA_CENTROIDS[name];

// Where a contact sits on the WORLD map. Returns { country, lat, lng } or null.
// A contact whose card printed no country but a recognisable Indian city counts
// as India, so both maps describe the same set of contacts.
export function resolveCountry(city, state, country) {
  const co = norm(country);
  let name = '';

  if (co) {
    name = COUNTRY_ALIASES[co] || (centroidFor(titleCase(co)) ? titleCase(co) : '');
    // Last resort: a case-insensitive scan of the known country names.
    if (!name) {
      const hit = [...Object.keys(COUNTRY_CENTROIDS), ...Object.keys(EXTRA_CENTROIDS)]
        .find(k => norm(k) === co);
      if (hit) name = hit;
    }
  } else if (resolveCoordinates(city, state, '')) {
    // Cards printed for a domestic audience routinely omit the country.
    name = 'India';
  }

  const coords = centroidFor(name);
  return coords ? { country: name, lat: coords[0], lng: coords[1] } : null;
}

const titleCase = (s) => s.replace(/\b\w/g, ch => ch.toUpperCase());

// Equirectangular projection for the world map. Outlines and bubbles both go
// through this, so they cannot drift apart.
export function projectOnWorld(lat, lng) {
  return { x: lng - WORLD_BOX.lngMin, y: WORLD_BOX.latMax - lat };
}

// ---------------------------------------------------------------------------
// Placing data on the doodle map
//
// The base map (lib/india-map.js) is hand-drawn, not a projection, so lat/lng is
// mapped onto it by an affine that was least-squares fitted against real India
// geometry. The artist traced closely: coastal cities land within ~5-30 km of
// the drawn coastline (Mumbai 21 km, Chennai 8 km, Surat 3 km), which is well
// under one bubble radius at display size.
//
// Re-fit the affine (and re-render it to an image and LOOK at it) if the artwork
// is ever swapped — the numbers below are specific to this drawing.
// ---------------------------------------------------------------------------
